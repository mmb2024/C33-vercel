		...
		{#each users as user (user.id)}
			<div class="flex items-center justify-between py-3">
				<div class="flex items-center space-x-4">
						<p class="font-medium leading-none">{user.name}
							<span class="font-medium leading-none">{user.lastname}</span>
							<span class="text-sm text-gray-500">{user.email}</span>
							<span class="text-sm text-gray-500">{user.message}</span>
							<span class="text-sm text-gray-500">{timeAgo(user.createdAt)}</span>
						</p>
				</div>
			</div>
		{/each}
